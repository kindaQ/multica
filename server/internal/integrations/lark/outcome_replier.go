...
func (r *LarkOutcomeReplier) Reply(ctx context.Context, inst Installation, msg InboundMessage, res DispatchResult) {
	...
	case OutcomeControl:
		if res.Message != "" {
			if err := r.sendControlText(ctx, inst, msg, res.Message); err != nil {
				r.log.Warn("lark outcome replier: control reply failed",
					"installation_id", uuidString(inst.ID),
					"chat_id", string(msg.ChatID),
					"err", err.Error(),
				)
...
func (r *LarkOutcomeReplier) sendControlText(ctx context.Context, inst Installation, msg InboundMessage, text string) error {
	if msg.ChatID == "" {
	...
	return sendWithThreadFallback(r.log, "send control text", inboundReplyTarget(msg), func(t ReplyTarget) error {
