...
func writeAvailableCommands(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("- `multica feishu push --content \"...\" --idempotency-key <stable-key> [--issue-id <issue-id>] [--installation-id <id>]` — proactively message the user through an active Feishu Bot shared with your squad. The server automatically selects the Bot when exactly one is available to you; specify `--installation-id` only when it reports multiple choices. With `--issue-id`, a quoted reply returns to this agent in that issue and the pushed content is persisted as this run's final issue comment. After a successful issue-routed push, do NOT call `multica issue comment add` and do NOT post a delivery receipt or message ID. Without `--issue-id`, the reply returns to this agent's chat session. Use a new stable idempotency key for each distinct message.\n\n")
...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	if ctx.IsSquadLeader {
		b.WriteString("5. **Post your final results as a comment** unless step 4 successfully sent the final result with `multica feishu push --issue-id` (that push already persisted the one final comment), or your outcome is `no_action` — in that case, calling `multica squad activity <issue-id> no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment announcing no_action, a Feishu delivery receipt/message ID, or that you are exiting silently. Otherwise post with `multica issue comment add` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`).\n")
	} else {
		b.WriteString("5. **Post your final results as a comment — this step is mandatory unless step 4 successfully sent the final result with `multica feishu push --issue-id`**: an issue-routed push already persists the one final comment, so do NOT also call `multica issue comment add` or post a delivery receipt/message ID. Otherwise post with `multica issue comment add` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). In Reply mode this step is also conditional on the reply rule below.\n")
	}
	...
	b.WriteString("- **Decide whether a text reply is warranted.** If you produced actual work this turn, post the result via step 5. If the triggering comment is only an acknowledgment / thanks / sign-off and you produced no work, do NOT post a comment. Instead, choose one suitable emoji and make your entire final output exactly `REACTION: <emoji>` (for example `REACTION: 👍` or `REACTION: 😂`). The server adds that Reaction to the triggering comment without creating another comment or sending a Feishu message.\n")
	if ctx.IsSquadLeader {
