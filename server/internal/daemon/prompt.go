...
func buildCommentPrompt(task Task, provider string) string {
	...
	if task.TriggerCommentContent != "" {
		...
		if task.TriggerAuthorType == "agent" {
			b.WriteString("⚠️ The triggering comment was posted by another agent. Decide whether a text reply is warranted. If you produced actual work this turn (investigated, fixed something, answered a real question), post the result as a normal reply. If the triggering comment was only an acknowledgment, thanks, or sign-off AND you produced no work, do not post a comment. Choose one suitable emoji and make your entire final output exactly `REACTION: <emoji>`; the server adds it to the triggering comment without creating a comment or triggering another run. If you do post a text reply, do not @mention the other agent as a sign-off (that re-triggers them and starts a loop).\n\n")
		}
