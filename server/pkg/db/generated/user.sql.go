...
VALUES ($1, $2, $3)
RETURNING id, name, email, avatar_url, created_at, updated_at, onboarded_at, onboarding_questionnaire, cloud_waitlist_email, cloud_waitlist_reason, starter_content_state, language, profile_description, timezone, default_workspace_id
`
...
SELECT id, name, email, avatar_url, created_at, updated_at, onboarded_at, onboarding_questionnaire, cloud_waitlist_email, cloud_waitlist_reason, starter_content_state, language, profile_description, timezone, default_workspace_id FROM "user"
WHERE id = $1
`
...
const replaceDefaultWorkspaceOnMembershipRemoval = `-- name: ReplaceDefaultWorkspaceOnMembershipRemoval :one
UPDATE "user" u
SET default_workspace_id = (
    ...
    updated_at = now()
WHERE u.id = $1
  AND u.default_workspace_id = $2
RETURNING id, name, email, avatar_url, created_at, updated_at, onboarded_at, onboarding_questionnaire, cloud_waitlist_email, cloud_waitlist_reason, starter_content_state, language, profile_description, timezone, default_workspace_id
`

type ReplaceDefaultWorkspaceOnMembershipRemovalParams struct {
	UserID             pgtype.UUID `json:"user_id"`
	RemovedWorkspaceID pgtype.UUID `json:"removed_workspace_id"`
}

func (q *Queries) ReplaceDefaultWorkspaceOnMembershipRemoval(ctx context.Context, arg ReplaceDefaultWorkspaceOnMembershipRemovalParams) (User, error) {
	row := q.db.QueryRow(ctx, replaceDefaultWorkspaceOnMembershipRemoval, arg.UserID, arg.RemovedWorkspaceID)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Name,
...
func (q *Queries) SetDefaultWorkspace(ctx context.Context, arg SetDefaultWorkspaceParams) (User, error) {
	row := q.db.QueryRow(ctx, setDefaultWorkspace, arg.WorkspaceID, arg.UserID)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Name,
...
const setDefaultWorkspaceIfUnset = `-- name: SetDefaultWorkspaceIfUnset :one
UPDATE "user"
SET default_workspace_id = $1,
    updated_at = now()
WHERE id = $2
...
type SetDefaultWorkspaceIfUnsetParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	UserID      pgtype.UUID `json:"user_id"`
}

func (q *Queries) SetDefaultWorkspaceIfUnset(ctx context.Context, arg SetDefaultWorkspaceIfUnsetParams) (User, error) {
	row := q.db.QueryRow(ctx, setDefaultWorkspaceIfUnset, arg.WorkspaceID, arg.UserID)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Name,
